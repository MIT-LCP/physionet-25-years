...
def combine_exporter_tables(folder, pattern, start_year):
    ...
    for year in list(range(int(start_year), 2024)):
        ...
        if df_combined is None:
            ...
            # NOTE: getting a lot of DtypeWarning: Columns (#,#) have mixed
            # types, consider specifying the dtype for
...
def get_nih_exporter_project_leader_names(projects_folder, start_year):
    ...
    df_projects = combine_exporter_tables(
        projects_folder, "RePORTER_PRJ_C_FY", start_year
    )
    ...
    project_formatted_names = (
        df_projects["project_formatted_name"].str.lower().dropna().tolist()
    )
...
def get_nih_exporter_publication_authors(publications_folder, start_year):
    ...
    df_publications = combine_exporter_tables(
        publications_folder, "RePORTER_PUB_C_", start_year
    )
    ...
    publication_formatted_names = (
        df_publications["publication_formatted_name"].str.lower().dropna().tolist()
    )
...
def check_nih_exporter_hits(
    df_pn_users, projects_folder, publications_folder, start_year
):
    """
    ...
    # df_pn_users = df_pn_users.iloc[1:11].copy() # For looping through
    # fewer users during debug
    ...
    project_formatted_names = get_nih_exporter_project_leader_names(
        projects_folder, start_year
    )

    # Get the list of authors from publications associated with the NIH
    # exporter projects
    publication_formatted_names = get_nih_exporter_publication_authors(
        publications_folder, start_year
    )
    ...
    print(
        "Searching for PhysioNet users who are PIs that were awarded grants in NIH Exporter..."
    )
    project_results = Parallel(n_jobs=-1)(
        delayed(best_jaro_winkler_match)(user_name, project_formatted_names)
    ...
    print(
        "Searching for PhysioNet users who are published authors from NIH Exporter..."
    )
    publication_results = Parallel(n_jobs=-1)(
        delayed(best_jaro_winkler_match)(user_name, publication_formatted_names)
        for user_name in tqdm(user_names)
    )
